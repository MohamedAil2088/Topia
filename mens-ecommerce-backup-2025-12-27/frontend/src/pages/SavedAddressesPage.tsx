    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                Swal.fire({ icon: 'success', title: 'Address Updated', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
            } else {
                await api.post('/users/address', addressData);
                Swal.fire({ icon: 'success', title: 'Address Added', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
            }
        ...
        <div className="min-h-screen bg-gray-50/50">
            <div className="container mx-auto px-4 py-12">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
                    <div>
                        <h1 className="text-4xl font-black font-display text-primary-900 tracking-tight">Saved Addresses</h1>
                        <p className="text-gray-500 font-medium mt-1">Manage your shipping destinations for faster checkout</p>
                    </div>
                    {!showForm && (
                        <button
                    ...
                    <div className="lg:w-3/4 space-y-8">
                        {showForm && (
                            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden animate-fade-in">
                                <div className="p-8 md:p-10 border-b border-gray-50 flex justify-between items-center">
                                    <h3 className="text-2xl font-black text-gray-900">{editingId ? 'Edit Address' : 'New Delivery Address'}</h3>
                                ...
                                <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-8">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">ZIP / Postal Code</label>
                                            <input
                                                required
                                                value={zipCode}
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        />
                                        <label htmlFor="isDefault" className="text-sm font-bold text-gray-700">Set as default delivery address</label>
                                    </div>
                                    <button
                                        type="submit"
                                    ...
                                    >
                                        {editingId ? 'Save Architecture' : 'Confirm Destination'}
                                    </button>
                                </form>
                            </div>
                        )}
                        ...
                        {loading ? (
                            <div className="p-32 flex flex-col items-center justify-center bg-white rounded-[2.5rem] border border-gray-100 shadow-sm">
                                <Loader />
                                <p className="mt-8 text-gray-400 font-bold animate-pulse uppercase tracking-[0.3em] text-[10px]">Accessing Vault...</p>
                            </div>
                        ) : addresses.length === 0 ? (
                            !showForm && (
                                <div className="text-center py-32 bg-white rounded-[3rem] border border-gray-100 shadow-sm px-10">
                                    ...
                                    <h2 className="text-4xl font-black text-gray-900 mb-4 tracking-tight">Empty Vault</h2>
                                    <p className="text-gray-500 mb-12 max-w-sm mx-auto font-medium leading-relaxed">You haven't saved any delivery destinations. Add your first address to accelerate your next checkout.</p>
                        ...
                        <div className="bg-gradient-to-br from-primary-950 to-gray-900 rounded-[3rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl">
                            <div className="relative z-10">
                                <h3 className="text-3xl font-black mb-6 tracking-tight">Elite Shipping Pipeline</h3>
                                <p className="text-gray-400 font-medium leading-relaxed max-w-lg">
                                    Your addresses are encrypted and secured. Having multiple addresses allows you to switch between home, office, and travel destinations with a single tap during checkout.
                                </p>
                            </div>
